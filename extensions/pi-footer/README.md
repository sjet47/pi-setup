...
| 段 | 定义 |
|----|------|
| `⚡Nt/s` | 当前消息的生成速率（首字之后 → `now`），再做 EMA（0.15 权重、80ms 节流）平滑。EMA **在每条消息的 `messageStart` 清零**，所以显示的永远是这一条消息自己的速率；分子与窗口同窗（thinking→text 不重置计时，也不把已计入的 thinking token 踢出分子）；`message_end` 用整条消息的窗口重算并**替换** EMA（冻结值≈这条消息的真实均值）。每条消息首字后有 **500ms 暖机**：期间不取样（第一个分片的 token 是窗口开始前生成的，早期样本量的是分片大小而不是速度），暖机后的第一个样本是整段窗口均值，直接作为 EMA 起点。新消息拿到第一个样本之前，**继续显示上一条消息的速率**（不会退回 `⚡…`）；run 的第一条消息没有可沿用的值，`⚡…` 会多停约 0.5s |
| `🔧` | 本 run 的工具调用次数 |
...
- **暖机期别去掉**：没有它，首个分片很小时（比如工具返回后的新消息只先出 2 个字）第一个样本会读成 ~10t/s，EMA 以 0.15 权重要 1s 多才爬回来——看起来像「工具调用后 TPS 掉了」，其实跟工具耗时无关。100ms 下限现在只在 `message_end` 的最终样本（很短的消息）里还起作用。
