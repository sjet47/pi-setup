...
| 段 | 定义 |
|----|------|
| `⚡Nt/s` | 当前消息**纯生成窗口**（首字之后）的 token 估算 / 时长，再做 EMA（0.15 权重、80ms 节流）平滑。首个样本会明显偏高：窗口被夹在下限 50ms，随后收敛 —— 与上游 pi-tps 一致 |
...
TTFT 与耗时都按「本条消息的请求发出时刻」计算，因此消息结束后数值冻结、不会随 `now` 继续增长。token 在流式期间按字符估算（thinking /4、正文 /3.5），`message_end` 拿到真实 usage 后重算一次并并入 run 累计。
...
单测 20 项全过：TpsTracker 时序（TTFT / think tokens / run 累计 / 冻结 / 换消息清零 / 忽略 turn 外的请求时刻）、降级顺序（逐步丢段到只剩核心、核心也放不下则整段消失）、边框宽度在 1..150 列 × 有无 name × 有无 status × 有无 stats 全部守恒。
