...
| 段 | 定义 |
|----|------|
| `⚡Nt/s` | 当前消息的生成速率（首字之后 → `now`），再做 EMA（0.15 权重、80ms 节流）平滑。EMA **在每条消息的 `messageStart` 清零**，所以显示的永远是这一条消息自己的速率；分子与窗口同窗（thinking→text 不重置计时，也不把已计入的 thinking token 踢出分子）；`message_end` 用整条消息的窗口重算并**替换** EMA（冻结值≈这条消息的真实均值）。每条消息首字后有 **500ms 暖机**（第一个分片的 token 是窗口开始前生成的，早期样本量的是分片大小而不是速度）：暖机期窗口向前借上一条消息的**尾段**——`tps = (本条 token + 尾段 token) / (本条已过时间 + 尾段时长)`，尾段取上一条消息生成期的最后 1s（生成不足 1s 就取整段，不把 prefill 算进去），token 按上一条最终计数缩放到同一单位；暖机结束时的值作为 EMA 起点，之后只用本条自己的窗口。新消息首字之前（prefill）**继续显示上一条消息的冻结值**（不会退回 `⚡…`）；run 的第一条消息没有尾段，暖机期不取样，`⚡…` 会多停约 0.5s，暖机后以整段窗口均值作 EMA 起点 |
| `🔧` | 本 run 的工具调用次数 |
...
- **速率是「当前消息」的速率**：EMA 在 `messageStart` 清零（prefill 期间展示层靠 `heldTps` 沿用旧值，暖机期靠尾段过渡；别为了「不闪」把 EMA 改回跨消息延续）、分子与窗口同窗、`message_end` 用整条消息重算。别把 EMA 改成跨消息累积，也别让窗口在 thinking→text 时重置（前者会拖进上一条消息的速率，后者会在分母掉回 100ms 下限时把分子里的 thinking token 放大成尖峰）。
- **暖机期别去掉**：没有它，首个分片很小时（比如工具返回后的新消息只先出 2 个字）第一个样本会读成 ~10t/s，EMA 以 0.15 权重要 1s 多才爬回来——看起来像「工具调用后 TPS 掉了」，其实跟工具耗时无关。100ms 下限现在只在 `message_end` 的最终样本和极短消息的尾段里还起作用。
- **尾段只借生成期，且只在暖机期借**：工具执行、prefill 的时间都不进分母；尾段为 0 token 时不采用（保留更早的尾段），否则会把下一次暖机拉低。前后两条速率接近时全程连续；速率差很大时（如 1000→100t/s）约 2s 过渡到新值，`message_end` 仍然用本条自己的窗口重算冻结值。
