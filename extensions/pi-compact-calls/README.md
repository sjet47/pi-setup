...
| 情况 | 渲染 |
|---|---|
| **一轮里连续**（中间没有可见正文）的工具调用 | 合成一块 —— 同一条消息里的并行批、跨多条消息的多步调用都算；块头显示 `N 次工具调用 · 总耗时` 与状态（`⠋` 进行中 / `✓` 全部成功 / `✗` 有失败） |
...
| 折叠态 | 块头 + 1 个工具行 + `… 另有 N 次调用 (Ctrl+O 展开)` |
| 展开态（`Ctrl+O`） | 每个工具一行（带树杈）+ 结果预览（最多 5 行 + `… 另有 N 行`），运行中也能看到流式输出 |
...
- 3 步各占一条消息、中间无可见正文 → **合成为一块**（`✓ 3 次工具调用 · 1.8s` + 正在跑/最新一行 + `另有 2 次`）✓
- 中间插入可见正文 → 断成两块（`echo K1` / 正文 / `echo K2`）✓
- 并行批里有一个 `sleep 20` 且不是最后一个 → 运行中显示仍在跑的那一个；全部完成后回落到最后一个 ✓
- 一条消息内并行 5 次 bash → 运行中 `⠋ 5 次工具调用 · 15.1s` + `✓ bash: echo N5` + `… 另有 4 次调用 (Ctrl+O 展开)`；完成后 `✓ 3/5 次工具调用 · …` 稳定不消失；`Ctrl+O` 展开显示全部 5 个 ✓
