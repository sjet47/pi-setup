...
块头只报「这批调用干了什么」（几个、哪些工具、纯工具耗时），**不报成败**：没有 `✓`/`✗`/`○` 图标，也没有 `M failed` 与失败错误末行 —— 成、败、中断在块头上一视同仁（活动行与工具行仍带结果状态，展开后每个调用各自的 `✓`/`✗` 也都在）。
...
| 块头（仅 ≥ 2 个调用） | `图标 N tool calls (类型分布) · 耗时 · Ctrl+O to expand`。类型分布只在块里混了 ≥ 2 种工具时出现；`Ctrl+O to expand` 只在折叠态出现 |
| 块封口后 | 活动行收掉，只留块头那一行（即上列的统计行）|
| 块头排版 | 宽度不够时按 提示 → 类型分布 → 耗时 的顺序丢弃，计数永远保留 |
| 块头图标 | 只表达「还在进行」：`⠋` 有调用在跑；`⠿`（muted，静态，不跑定时器）块还开着但没有调用在跑 = 模型正在生成下一个调用。**块封口后没有任何图标** —— 成功 / 有失败 / 有调用始终没执行在块头上一视同仁（这是 2026-09-22 的决定，成败信息只留在工具行与展开态） |
...
| 中断（Esc） | 没执行的调用不会显示 `✓`：pi 给出 `Operation aborted` 结果的显示 `✗`，没有任何结果的保持 `○`（只管工具行 —— 块头永远不带状态图标） |
...
- 单工具块的工具行在执行完后就显示 `✓`（那个调用确实成功了）；块头**永远**不显示`✓`/`✗`/`○`（成败不在块头上表达）。
- 块头不再报失败，所以**封口后的折叠块光看块头看不出这一批失败过**（要 `Ctrl+O` 展开或看单工具块/活动行）。若想要回失败信息，最小改法是在 `renderGroupBlock` 的封口分支重新传一个尾串（`format.ts` 的 `errorTail()` 与 `MIN_ERROR_TAIL_WIDTH` 都还在，工具行仍在用）。
...
用户定的规则：块头的 tool call 总结不再区分成功 / 失败 / 中断，**只保留「还在进行」的 progress indicator**。改动：`HeaderState` 从 `running | idle | ok | failed | incomplete` 收敛为 `running | idle | settled`（成败判定整段删掉），`HeaderParts` 去掉 `failed` 与 `errorTail`，封口头不再带图标与颜色区分（统一 muted），`collapsedErrorTail()` 删除；工具行与展开态一行未动。

单测 `node --test tests/*.test.ts` 39 项全过（重写 `headerState` / `composeHeader` 用例：新增「封口头不含 `✓`/`✗`/`○`/`failed`/`—`」与「running/idle 保留 `⠋`/`⠿`」；因少了 ` · 1 failed` 那 11 列，提示的进位点前移，宽度用例如实更新）；本扩展 typecheck 0 报错（仓库里 pi-execution-time / pi-shots 的 7 个既有报错与本次无关，未动）。

本轮是纯排版改动，未做 tmux 抓屏：宽度守恒与降级顺序由单测守住，界面在真实会话里由用户 `/reload` 自行确认。
